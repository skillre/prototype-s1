...
1. **不是"自己写了一个图库"**：节点位置由 `view-model.ts` 的 `attackChainLayout` **算出来**
   （按图的形状分层 + 层内重心排序，落点是"第几排、排内第几、占哪几列"），
   不散落在 JSX、也不是视觉常量；谁被画出来、画成什么状态**全部由游标派生**。
   > **2026-09-17 修订（组件批次 3）**：这里原先是一张手写的构图表 `ATTACK_CHAIN_POSITION`
   > 加一组"空格子"。它在**坐标层面**是对的（两个节点不落在同一个坐标上），但在**像素层面**
   > 是错的：画布 444×444，节点宽 202px、高 98–216px，而格子间距（0.48 × 444 ≈ 213px）
   > 小于节点自己的宽或高 —— 六个节点在四个格子里视觉上互相压（截图实拍）。
