 ...
 * 战情室骨架 —— **实测尺寸的唯一一份**。
 *
 * 出处：工作区根目录 `S1-设计稿实测与实现规格.md` §二（从 `s1-ai-native-console.pen`
 * 逐帧读出，2026-09-16）。这些不是审美取值，是**实测事实**；改一个数字就是改设计稿的骨架，
 * 所以它们只在这里出现一次，并由 `tests/s1-console.spec.ts` 逐值比对规格文档，
 ...
 *    **实测值**（`S1-设计稿实测与实现规格.md` 是骨架的权威），并把 pack 的
...
export const CONSOLE_GEOMETRY = {
  ...
  headerHeight: 76,
  /** 底栏高度（`layoutFacts.footerHeight`）：⑨ 问 S1（640）+ ⑭ 报告。 */
  footerHeight: 130,
  ...
  footerPadding: 12,
  footerGap: 12,
  /** ⑨ 问 S1 的宽度（`layoutFacts.askS1Width`）。 */
  askS1Width: 640,
} as const
